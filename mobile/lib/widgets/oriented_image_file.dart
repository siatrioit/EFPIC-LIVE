...
class OrientedImageFile extends StatefulWidget {
  ...
  final String path;
  /// NEF/RAW ceļš, ja [path] ir `_emb.jpg` priekšskatījums.
...
class _OrientedImageFileState extends State<OrientedImageFile> {
  ...
  void didUpdateWidget(OrientedImageFile oldWidget) {
    ...
    if (oldWidget.path != widget.path ||
        oldWidget.rawSourcePath != widget.rawSourcePath) {
      _loaded = false;
      _load();
    }
  }
  ...
  Future<void> _load() async {
    final o = await ImageOrientation.readExifForDisplay(
      widget.path,
      rawSourcePath: widget.rawSourcePath,
    );
    if (!mounted) return;
    setState(() {
      _orientation = o;
